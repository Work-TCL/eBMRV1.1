...
NOT_APPLICABLE_STATUSES = ("N/A", "NOT_APPLICABLE")  # both spellings occur in TEST_CASE_LIBRARY.csv
...
for m in status["modules"]:
    ...
    for rid, sts in req_state.get(m["module"], {}).items():
        if rid in m["requirements_state"]:
            # A "not applicable" disposition is a real, evidenced, reasoned execution outcome (the
            # pre-written case's condition doesn't exist for this operation -- see actual_result on the
            # row), not an unexecuted/open item. It must count the same as PASS toward VERIFIED, provided
            # at least one case for the requirement was an actual PASS (all-N/A alone is not verification).
            if any(s == "PASS" for s in sts) and all(s == "PASS" or s in NOT_APPLICABLE_STATUSES for s in sts):
