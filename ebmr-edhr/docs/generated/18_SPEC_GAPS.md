...
status: RESOLVED (all 27 pairs; code+tests written 2026-09-11, pytest evidence pending)
```
...
so only the unsigned/signed states apply). Not yet executed against `pytest` — see
`status/build-status.json` for current evidence status.
...
status: RESOLVED (all 5 pairs; code+tests written 2026-09-11, pytest evidence pending)
```
