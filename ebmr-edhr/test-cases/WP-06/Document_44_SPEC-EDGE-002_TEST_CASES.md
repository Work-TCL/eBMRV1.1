...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_modbus_rtu.py` (6 tests: connect+read holding register FLOAT32, read coil, wrong-unit-id error, out-of-range `PROTOCOL_EXCEPTION`, health check, nonexistent-port `ENDPOINT_UNREACHABLE`), against a real `pymodbus.server.ModbusSerialServer` (RTU framer) bound to one end of a real `socat`-created virtual serial pty pair, with `ModbusDriver(variant="MODBUS_RTU")` connected to the other end -- real RTU address byte + PDU + CRC16 framing crosses the wire exactly as on a physical bus. `socat` is test-only tooling (already present in this environment), not an `edge/pyproject.toml` runtime dependency.  |  **Defect:** —
...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_mqtt.py` (`MqttDriver.connect`/`read_once` and `MqttPlugin.poll`) against a real local `mosquitto` broker subprocess (`allow_anonymous`, ephemeral port); TLS-capable (`mqtts://` scheme routes to `ssl.create_default_context()`), auth (username/password), QoS (publish/subscribe qos param), retained-flag handling (`test_retained_message_is_marked_stale_not_good`) and schema/version-checked decoding (`plugins/mqtt/decoders.py`) all exercised for real.  |  **Defect:** —
...
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** N/A  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** N/A -- this pre-written case's condition ("two actors submit conflicting commands on the same aggregate", expected error `STALE_VERSION`) is the generic GxP-command-handler template and does not exist for an MQTT protocol driver: `MqttDriver`/`MqttPlugin` are a one-way ingestion connector with no writable aggregate, no `expected_version` and no concept of two actors racing a write. DRV-FR-010's real required behaviour (broker TLS/auth/QoS/retained-flag/session policy) is proven positively by TC-044-010-01 against a real mosquitto broker; there is no concurrency-under-write scenario Document 44 section 7 asks this driver to satisfy.  |  **Defect:** —
...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_mqtt.py`: `test_schema_version_mismatch_raises_payload_schema_invalid`, `test_malformed_json_raises_payload_schema_invalid` and `test_unknown_decoder_raises_payload_schema_invalid` all prove `plugins/mqtt/decoders.py` rejects a payload against a real published MQTT message (schema_version mismatch, invalid JSON, unregistered decoder id) with `PayloadSchemaInvalid` rather than best-effort parsing.  |  **Defect:** —
...
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** N/A  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** N/A -- generic GxP-command-handler concurrency template (see TC-044-010-02's identical reasoning); `MqttDriver`/`MqttPlugin` have no writable aggregate or `expected_version` for two actors to race. DRV-FR-011's real required behaviour is proven positively by TC-044-011-01.  |  **Defect:** —
...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_snmp.py` (8 tests) against a real local SNMPv3 agent (`pysnmp.entity`'s own `GetCommandResponder`, run on its own background thread/event loop): real USM auth+priv GET (`sysDescr.0`/`sysUpTime.0`, real BER encoding and AES privacy encryption over a real UDP socket), unknown-username -> `AuthFailed`, known-user-wrong-key -> `EndpointUnreachable` (a real, confirmed USM anti-enumeration property, documented in `plugins/snmp/driver.py::classify_snmp_error`), out-of-VACM-view OID -> `SourceMappingNotFound` (`NoSuchObject`/`NoSuchInstance` varbind detection), and `SnmpPlugin.poll()` end-to-end.  |  **Defect:** —
...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_serial.py`: framing/checksum test vectors (`test_known_test_vector`, `test_encode_decode_roundtrip`, `test_decode_frame_rejects_bad_checksum`/`missing-delimiters`) plus real TCP-transport (real local `TCPLineServer`) and real serial-transport (`socat` virtual pty pair, `test_serial_transport_reads_valid_frame`) reads via `plugins/serial/{framing,driver,plugin}.py`.  |  **Defect:** —
...
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_serial.py::test_replayed_sequence_is_marked_uncertain`: a frame whose 4-hex-digit sequence number was already seen is detected as a replay. Document 44 section 9's stable driver error taxonomy has no `REPLAY_DETECTED` code (that vocabulary belongs to the GxP mutation layer, not a protocol driver) -- this driver surfaces a detected replay as `quality_hint=UNCERTAIN` (Document 43 section 5's existing quality category for a present-but-not-trustworthy-as-new value), documented in `plugins/serial/driver.py`, not the literal `STALE_VERSION`/`REPLAY_DETECTED` codes this generic template names.  |  **Defect:** —
...
- **Depends on:** TC-044-013-01
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_serial.py::test_read_timeout_then_fresh_read_never_fabricates_a_value`: after a `ReadTimeout` (no frame arrived), the driver raises rather than returning a cached value, and the next genuine read returns the real newly-arrived value -- the driver never blindly re-creates or fabricates state from an uncertain outcome.  |  **Defect:** —
...
- **Evidence to capture:** request/response, aggregate before/after, audit event id, outbox row, screenshot where UI-driven
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest: `edge/tests/test_plugin_rest.py` (9 tests, real local `http.server` instrument stand-in -- bearer-auth GET polling, dotted JSON field extraction, 401/malformed-JSON/500/timeout handling) and `edge/tests/test_plugin_file_adapter.py` (4 tests, real local drop-directory file import via `plugins/file/plugin.py` + the existing `plugins/common/file_ingest.py` mechanism).  |  **Defect:** —
...
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** N/A  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** N/A -- this pre-written case's condition ("re-deliver a previously accepted external message with the same source id", expected error `REPLAY_DETECTED`) does not apply to this driver: a REST GET is a fresh request/response each poll with no persisted external message identity to replay, and Document 44 section 9's stable taxonomy has no `REPLAY_DETECTED` code for a protocol driver. The file-import half already has real duplicate-safety (`test_poll_is_idempotent_across_restarts`: a file moved to `processed/` is never re-ingested by a fresh plugin instance), which is the closest real analogue and is proven positively by TC-044-014-01.  |  **Defect:** —
...
- **Depends on:** TC-044-014-01
- **Evidence to capture:** request/response with error code, unchanged aggregate proof, audit/security event
- **Status:** PASS  |  **Executed by:** claude-code  |  **Date:** 2026-09-15  |  **Actual result:** PASS -- exercised by real pytest in `edge/tests/test_plugin_rest.py::test_read_timeout`: a hung REST response raises `ReadTimeout` rather than the driver fabricating/reusing a cached value; the next real poll cycle performs a fresh GET and looks up the instrument's actual current state rather than blindly re-creating one.  |  **Defect:** —
