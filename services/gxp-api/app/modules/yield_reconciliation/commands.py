...
async def get_batch_summary(session: AsyncSession, batch_id: uuid.UUID) -> dict:
    ...
    return {
        ...
        "calculations": [
            {
                "id": str(c.id), "site_id": str(c.site_id), "calculation_type": c.calculation_type, "phase_code": c.phase_code,
                "scope_type": c.scope_type, "scope_id": str(c.scope_id) if c.scope_id is not None else None,
                "device_unit_id": str(c.device_unit_id) if c.device_unit_id is not None else None,
                "rule_object_id": str(c.rule_object_id) if c.rule_object_id is not None else None,
                "rule_evaluation_id": str(c.rule_evaluation_id) if c.rule_evaluation_id is not None else None,
                "input_refs": c.input_refs, "input_hash": c.input_hash,
                "theoretical_quantity": str(c.theoretical_quantity) if c.theoretical_quantity is not None else None,
                ...
                "min_percent": str(c.min_percent) if c.min_percent is not None else None,
                "max_percent": str(c.max_percent) if c.max_percent is not None else None,
                "manual_source": c.manual_source, "manual_reason": c.manual_reason,
                "result": c.result, "state": c.state,
                "evaluated_by_user_id": str(c.evaluated_by_user_id) if c.evaluated_by_user_id is not None else None,
                "evaluated_at": c.evaluated_at.isoformat() if c.evaluated_at is not None else None,
                "verified_signature_id": str(c.verified_signature_id) if c.verified_signature_id is not None else None,
                "verified_by_user_id": str(c.verified_by_user_id) if c.verified_by_user_id is not None else None,
                "verified_at": c.verified_at.isoformat() if c.verified_at is not None else None,
                "supersedes_id": str(c.supersedes_id) if c.supersedes_id is not None else None,
                "verified": c.state == "VERIFIED", "version": c.version, "created_at": c.created_at.isoformat(),
            }
            for c in calculations
        ],
        "reconciliations": [
            {
                "id": str(r.id), "site_id": str(r.site_id), "reconciliation_type": r.reconciliation_type, "item_ref": r.item_ref,
                ...
                "evaluated_by_user_id": str(r.evaluated_by_user_id) if r.evaluated_by_user_id is not None else None,
                "verified_signature_id": str(r.verified_signature_id) if r.verified_signature_id is not None else None,
                "verified_by_user_id": str(r.verified_by_user_id) if r.verified_by_user_id is not None else None,
                "verified_at": r.verified_at.isoformat() if r.verified_at is not None else None,
                "supersedes_id": str(r.supersedes_id) if r.supersedes_id is not None else None,
