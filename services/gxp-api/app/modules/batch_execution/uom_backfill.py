...
`ebmr.gxp_batch.target_uom_id` (the expand step, migration `0051`, added the column nullable).
`ebmr.gxp_batch` is mutable (UPDATE granted, migration `f264272f2f0b` 0012), so a backfill is possible.
...
async def backfill_batches(
    ...
    query = (
        select(Batch)
        .where(Batch.target_uom_id.is_(None), Batch.target_uom.is_not(None))
        .order_by(Batch.id)
        .limit(batch_size)
    )
    ...
    for row in rows:
        try:
            resolved = await rules_service.resolve_uom(session, row.target_uom)
        except UomUnknownError:
            unmatched += 1
            last_id = row.id
            continue
        row.target_uom_id = resolved.uom_id
        matched += 1
        last_id = row.id
    await session.flush()
