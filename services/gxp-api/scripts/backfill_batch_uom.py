"""SG-146 (remainder, module 4 of 8) backfill CLI for `ebmr.batches.uom_id` (migration `0051`).
...
Usage: python scripts/backfill_batch_uom.py [--batch-size N]
"""
...
from app.modules.batch.uom_backfill import backfill_batches
...
async def main(batch_size: int) -> None:
    ...
    print(f"batches: processed={total_processed} matched={total_matched} unmatched={total_unmatched}")
