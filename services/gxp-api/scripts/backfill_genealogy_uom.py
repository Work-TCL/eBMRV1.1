"""SG-146 (remainder, module 4 of 8) backfill CLI for `ebmr.genealogy_edge.uom_id` (migration `0051`).
...
Usage: python scripts/backfill_genealogy_uom.py [--batch-size N]
"""
...
from app.modules.genealogy.uom_backfill import backfill_genealogy_edges
...
async def main(batch_size: int) -> None:
    ...
    while True:
        async with SessionLocal() as session:
            async with session.begin():
                result = await backfill_genealogy_edges(session, batch_size=batch_size, after_id=after_id)
    ...
    print(f"genealogy_edge: processed={total_processed} matched={total_matched} unmatched={total_unmatched}")
