"""SG-146 (remainder, module 4 of 8) backfill CLI for `ebmr.gxp_product_version.strength_uom_id`
(migration `0051`).
...
Usage: python scripts/backfill_product_master_uom.py [--batch-size N]
"""
...
async def main(batch_size: int) -> None:
    ...
    while True:
        async with SessionLocal() as session:
            async with session.begin():
                result = await backfill_product_versions(session, batch_size=batch_size, after_id=after_id)
    ...
    print(f"gxp_product_version: processed={total_processed} matched={total_matched} unmatched={total_unmatched}")
