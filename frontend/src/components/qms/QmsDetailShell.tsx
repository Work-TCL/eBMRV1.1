...
/** Shared frame for a QMS record detail page: title with live state, a back link, the transition
 * buttons, and the record's fact header. Body content (tabs, JSON panels) is passed as children. */
export function QmsDetailShell({
  recordNumber,
  state,
  subtitle,
  backHref,
      ...
      <PageHead
        title={
          <span className="flex items-center gap-3">
            {recordNumber} <WorkflowStatePill state={state} />
          </span>
        }
        subtitle={subtitle}
        action={
...
/** Submission plumbing shared by every QMS transition form: busy flag, error surfacing in the
 * codebase's `CODE: message` convention, and refresh-on-success. Keeps each action form to its fields. */
